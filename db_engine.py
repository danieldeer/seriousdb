...
from .config import DB_FILE,WAL_FILE,COMPACT_THRESHOLD,WAL_SYNC_ON_WRITE

DEFAULT_DB= {"default":"default"} #the default entry we will use to populate database when no DB_FILE found
...
class Db_Engine:
        ...
        with self.lock:
            ...
            #step2: If exists then try to load and if fails save a backup adn load fresh again
                ...
                try:
                    ...
                    os.replace(DB_FILE,backup) #atomically replace the backup file with corrupted DB_FILE
                    with open(DB_FILE,"w") as f:
                        json.dump(DEFAULT_DB,f)
                    self.state=dict(DEFAULT_DB)
            ...
            if os.path.isfile(WAL_FILE):
                with open(WAL_FILE, "r") as f:
                    for line in f:
                        if line.strip():
                            entry=json.loads(line) #loads is being used instead of load cause we are loading string(each line in WAL file) not a file object
        ...
        with self.lock:
            ...
            with open(WAL_FILE,"a") as f:
                ...
                if WAL_SYNC_ON_WRITE:
                    ...
                    os.fsync(f.fileno()) #force the OS to block execution until the bytes are stored into WAL file
    ...
    def _compact_locked(self):
        ...
        with open(tmp_file,"w") as f:
            ...
            f.flush() #push python buffer into OS
            os.fsync(f.fileno()) #force the OS to write it to physical disk.this is to make the operating system to actually write data from memory buffers to the physical disk, instead of leaving it sitting in a cache
