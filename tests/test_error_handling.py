...
class TestApiErrorResponses:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param("GET", "/db", {"key": "name"}, None, id="get"),
            pytest.param("PUT", "/db", {"key": "name"}, {"value": "Alice"}, id="put"),
            pytest.param("DELETE", "/db", {"key": "name"}, None, id="delete"),
            pytest.param("GET", "/db/all", {}, None, id="get-all"),
        ],
    )
    def test_unloaded_database_returns_503(self, client, method, url, params, json):
        unloaded = Cache()
    ...
    def test_rejected_query_parameter_returns_a_structured_422(self, client):
        response = client.put("/db", params={"key": ""}, json={"value": "Alice"})
        assert response.status_code == 422
        assert response.json()["error"] == "request_validation_error"
        assert response.json()["detail"][0]["loc"] == ["query", "key"]
